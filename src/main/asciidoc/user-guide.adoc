...
Furthermore, you can customize using the Spring Boot configuration properties instead of `mybatis-thymeleaf.properties` as follow:

[source%nowrap,properties]
.src/main/resources/application.properties
----
mybatis.configuration-properties.use-2way=true # <1>
...
<2> Create a `ThymeleafLanguageDriverConfig` instance using `Properties` object that holds values of
    `mybatis.configuration.configuration-properties.{property key}` and
    register a `ThymeleafLanguageDriver` correspond with it.

<3> Set the `ThymeleafLanguageDriver` class as default scripting language driver
    at after registering a `ThymeleafLanguageDriver` instance instead of Spring Boot configuration properties

==== Full customizing a configuration
...
<3> Set the `ThymeleafLanguageDriver` class as default scripting language driver at after registering a `ThymeleafLanguageDriver` instance instead of Spring Boot configuration properties
